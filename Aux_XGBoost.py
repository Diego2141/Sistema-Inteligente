...
colors = {0.1: 'blue', 0.5: 'green', 0.95: 'red'}
for tau in quantiles:
    plt.plot(X_plot, predictions[tau], color=colors[tau], lw=2.5, 
             label=f'QR(τ={tau})')
# Shade 80% prediction interval
plt.fill_between(X_plot.flatten(), predictions[0.1], predictions[0.95],
...
def objective(trial):
    ...
    params = {
        'objective': 'quantile',
        'alpha': 0.95,  # Fix quantile (tune per quantile separately)
...
test_loss = np.where(residual_test >= 0, 0.95 * residual_test, -0.05 * residual_test).mean()
...
pred_upper = preds_corrected[0.95]
...
# Save best tau=0.95 model
final_models[0.95].save_model('qr_tau095_v2.txt')

# Load
loaded_model = lgb.Booster(model_file='qr_tau095_v2.txt')
