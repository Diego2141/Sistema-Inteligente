...
HMM_N_STARTS              = 10    # 1 = sin reinicios (original); ≥5 recomendado
...
def _transmat_reordenada(modelo, ss) -> np.ndarray:
    """
    Devuelve transmat_ reordenada según la convención 0=calma, 1=moderado,
    2=severo (mismo orden 'ss' que usa _predecir para las etiquetas). Sin este
    reordenamiento, transmat_ queda en el orden interno arbitrario de hmmlearn.
    Necesaria para guardar la matriz de transición que alimenta la simulación
    de paths (step006).
    """
...
def hmm_evolucion(
    ...
    La lógica NUEVA trabaja con fechas exactas (pd.Timestamp):
      ...
      - Si fechas_corte=None, las genera internamente a partir de HMM_INICIO
        y primer_ventana (en años), avanzando de año en año desde esa fecha.
    ...
    fechas_corte  : lista de pd.Timestamp con los cortes exactos. Si se pasa,
                    primer_ventana se ignora. Típicamente = train_end de cada
                    fold del walk-forward.
    ...
    if fechas_corte is not None:
        ...
        # Cortes anuales desde primer_corte hasta el fin de los datos
        cortes = []
        c = primer_corte
        while c <= flujo.index.max():
            cortes.append(c)
            c = c + relativedelta(years=1)
    ...
    print(f"\n  [EVOLUCIÓN] {desc_vol}+HMM por bloque | "
          ...
          f"primer_ventana={primer_ventana}a | winsor={WINSOR_ESTIMACION}")
...
def cargar_transmat(banco: str = BANCO, año_corte: int = None,
    ...
    """
    Carga transmat_hmm_<banco>.parquet y reconstruye la matriz 3x3 del fold
    pedido. Si año_corte=None, usa el fold más reciente (el de mayor año_corte)
    — el típico para simular paths hacia adelante desde hoy.
    """
...
def main():
    ...
    if GUARDAR_OBJETOS_SIMULACION:
        guardar_objetos_simulacion(evol, flujo)
    embig = cargar_embig()
