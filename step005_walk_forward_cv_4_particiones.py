...
# ── BOTÓN DE PARTICIÓN ──────────────────────────────────────────────────────
# Qué matriz leer y qué entidad modelar. Es el único bloque que hay que tocar
# para cambiar de corrida; todo lo demás se deriva.
#
#   PARTICION  None       -> matriz_features.parquet          (v1, sin partir)
#              "bbva"     -> matriz_features_particiones_bbva.parquet
#              "globales" -> matriz_features_particiones_globales.parquet
#
#   ENTIDAD    "SISTEMA"  -> el agregado, disponible en cualquier matriz
#              "FOCO"     -> FOCO_BBVA / FOCO_GLOBALES según la partición
#              "RESTO"    -> RESTO_BBVA / RESTO_GLOBALES
#
# Una entidad por corrida, a propósito: el modelo, los folds y las salidas
# quedan exactamente como estaban, y se revisan los resultados de una antes de
# lanzar la siguiente.
PARTICION = "bbva"
ENTIDAD   = "FOCO"
# ────────────────────────────────────────────────────────────────────────────
...
# El nombre real de la entidad dentro de la matriz. FOCO/RESTO solo existen si
# hay partición activa: pedirlos sobre la matriz de v1 es un error de config, y
# es mejor abortar acá que entrenar sobre un DataFrame vacío 40 minutos después.
if ENTIDAD == "SISTEMA":
    BANCO = "SISTEMA"
elif PARTICION:
    ...
    raise ValueError(
        f"ENTIDAD={ENTIDAD!r} requiere una PARTICION activa. "
        f"Con PARTICION=None la matriz solo tiene SISTEMA y los bancos "
        f"individuales, no FOCO ni RESTO.")
H_MIN   = 2
H_MAX   = 75
...
def _dir_modo() -> Path:
    ...
    # BANCO entra en la ruta. Varias salidas de diagnóstico no lo llevan en el
    # nombre del archivo (convergencia_{tau}.png, panel_{senal}_{tau}.png), así
