...
Los ejes Y están fijados al rango global para que la animación sea comparable entre fechas.

Requisitos:
  - ffmpeg en el PATH para MP4  (pip install / conda install ffmpeg)
...
DIR_MODELOS  = BASE_SISTEMA / "2. Output" / "modelos" / "eval"
...
def cargar_modelos(banco, dir_modelos):
    ...
    print(f"Modelo cargado : {metas[0].name}  ({len(cols_feat)} features)")
    return modelos, cols_feat, quantiles


# ── 2. Datos ──────────────────────────────────────────────────────────────────
def cargar_datos(banco, cols_feat):
...
def predecir_fecha(df, fecha_origen, medianas, cols_num, cols_feat, modelos):
    ...
    X = df_f[[c for c in cols_feat if c in df_f.columns]].copy()
    res = df_f[["h", "target"]].copy().reset_index(drop=True)
    for tau, model in modelos.items():
        res[f"q{int(tau*100):02d}"] = model.predict(X)
...
def precomputar(df, medianas, cols_num, cols_feat, modelos, fechas_validas):
    """Ejecuta la predicción para todas las fechas y devuelve lista de dicts."""
    ...
    for i, f in enumerate(fechas_sel, 1):
        ...
        hs       = res["h"].values
        real_mm  = res["target"].values / 1e6
        mask     = ~np.isnan(real_mm)
        ...
        cum_r   = np.where(mask, np.nancumsum(np.where(mask, real_mm, 0)), np.nan)
        cum_r[~mask] = np.nan

        frames.append({
            "fecha": fecha_origen,
            "hs": hs, "real": real_mm, "mask": mask, "h_max_r": h_max_r,
...
def rangos_globales(frames):
    y1_min = min(min(f["q01"].min(), f["real"][f["mask"]].min() if f["mask"].any() else 0)
                 for f in frames)
    y1_max = max(max(f["q99"].max(), f["real"][f["mask"]].max() if f["mask"].any() else 0)
                 for f in frames)
...
def animar(frames, ylim1, ylim3, banco):
    ...
    # Leyenda estática en ax1
    from matplotlib.patches import Patch
    from matplotlib.lines import Line2D
    legend_elements = [
        Patch(facecolor="steelblue", alpha=0.12, label="Q01–Q99 (98%)"),
        Patch(facecolor="steelblue", alpha=0.40, label="Q05–Q95 (90%)"),
    ...
    legend_cum = [
        Patch(facecolor="steelblue", alpha=0.12, label="Q01–Q99 acum."),
        Patch(facecolor="steelblue", alpha=0.40, label="Q05–Q95 acum."),
        Line2D([0], [0], color="crimson", lw=2, label="Mediana acumulada Q50"),
        Line2D([0], [0], color="black",   lw=2, label="Realizado acumulado"),
    ]
    ax3.legend(handles=legend_cum, loc="upper left", fontsize=9, framealpha=0.9)
    ...
    def update(i):
        ...
        # Restaurar formato de ejes tras cla()
        for ax, ylim in [(ax1, ylim1), (ax3, ylim3)]:
            ax.set_xlim(hs.min() - 1, hs.max() + 1)
            ax.set_ylim(*ylim)
            ax.axhline(0, color="black", lw=0.7, ls="--", alpha=0.35)
        ...
        # ── Subplot 1 ────────────────────────────────────────────────────
        ax1.fill_between(hs, f["q01"], f["q99"], alpha=0.12, color="steelblue")
        ax1.fill_between(hs, f["q05"], f["q95"], alpha=0.28, color="steelblue")
        ax1.plot(hs, f["q05"], color="steelblue", lw=1.0, ls=":", alpha=0.7)
        ax1.plot(hs, f["q95"], color="steelblue", lw=1.0, ls=":", alpha=0.7)
        ...
        # ── Subplot 3 (acumulado) ────────────────────────────────────────
        ax3.fill_between(hs, f["cum_q01"], f["cum_q99"], alpha=0.12, color="steelblue")
        ax3.fill_between(hs, f["cum_q05"], f["cum_q95"], alpha=0.28, color="steelblue")
        ax3.plot(hs, f["cum_q05"], color="steelblue", lw=1.0, ls=":", alpha=0.7)
        ax3.plot(hs, f["cum_q95"], color="steelblue", lw=1.0, ls=":", alpha=0.7)
        ax3.plot(hs, f["cum_q50"], color="crimson", lw=2.0, zorder=5)
        if f["mask"].any():
            hs_r = hs[f["mask"]]
            ax3.plot(hs_r, f["cum_r"][f["mask"]], color="black", lw=2, zorder=6)
            ax3.scatter(hs_r, f["cum_r"][f["mask"]], color="black", s=18, zorder=7)
        if f["h_max_r"] > 0:
            ax3.axvline(f["h_max_r"], color="red", lw=1.2, ls="--", alpha=0.7)
        ax3.legend(handles=legend_cum, loc="upper left", fontsize=9, framealpha=0.9)

        title.set_text(
            f"Fan Chart — {banco}  |  Origen: {f['fecha'].strftime('%d %b %Y')}  |  "
