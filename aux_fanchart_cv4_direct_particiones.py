# -*- coding: utf-8 -*-
"""
aux_fanchart_cv4_direct_particiones.py
======================================
Nueva versión de aux_fanchart_cv4_direct.py, pareada con
step005_walk_forward_cv_4_particiones.py.

Qué cambia respecto del original, que sigue intacto para el pipeline sin partir:

  · Botón PARTICION / ENTIDAD, el mismo que el de step005 particiones.
  · Lee la matriz de la partición correspondiente
    (matriz_features_particiones_<particion>.parquet).
  · Busca las predicciones bajo ETIQUETA = <BANCO>_<val>_<test>, p. ej.
    FOCO_BBVA_0.5_1, que es la carpeta que crea etiqueta_corrida() en step005.

Por qué archivo nuevo y no una modificación en el lugar: el original está
apareado con step005_walk_forward_cv_4.py, que escribe a fold_<modo>/ sin
subcarpeta de entidad. Cambiarlo dejaría sin leer todas las salidas ya
generadas con esa convención.

ADVERTENCIA heredada del original, sin corregir: el panel 3 acumula cada
trayectoria de cuantil por separado (np.cumsum sobre res["q01"], res["q50"],
etc.). La suma de medianas no es la mediana de la suma, y la suma de cuantiles
supone que todos los días salen en el mismo percentil a la vez. El centro queda
sesgado y la banda mucho más ancha que el intervalo verdadero del acumulado.
Ver la sección de errores estadísticos en CLAUDE.md.

Fan charts para el modelo cv4 DIRECT (step005_walk_forward_cv_4_particiones.py).
...
Cada fuente escribe en su propia carpeta (fan_<tipo>/<ETIQUETA>,
fan_<tipo>_val/<ETIQUETA>, fan_<tipo>_train/<ETIQUETA>), de modo que los PNG y el Excel de una nunca pisan
...
# ── BOTÓN DE PARTICIÓN ──────────────────────────────────────────────────────
# Tiene que coincidir con el de step005_walk_forward_cv_4_particiones.py: de
# esas constantes sale la carpeta donde ese script dejó las predicciones, así
# que si no coinciden el fan chart no las encuentra.
#
#   PARTICION  None | "bbva" | "globales"
#   ENTIDAD    "SISTEMA" | "FOCO" | "RESTO"
#
# VENTANA_VAL_AÑOS y VENTANA_TEST_AÑOS también deben replicar las de step005:
# forman parte del nombre de la carpeta porque una corrida con otra geometría
# de fold NO es comparable, y sin el sufijo se pisaban entre sí.
PARTICION = "bbva"
ENTIDAD   = "FOCO"
...
if ENTIDAD == "SISTEMA":
    ...
    raise ValueError(
        f"ENTIDAD={ENTIDAD!r} requiere una PARTICION activa. Con PARTICION=None "
        f"la matriz solo tiene SISTEMA y los bancos individuales.")


def _fmt_anios(x: float) -> str:
    """0.5 -> '0.5', 1.0 -> '1'. Misma definición que en step005."""
    return f"{x:g}"


# Espejo de etiqueta_corrida() de step005_walk_forward_cv_4_particiones.py.
# Las dos definiciones tienen que moverse juntas: es la clave de encuentro
# entre el script que escribe las predicciones y el que las grafica.
ETIQUETA = f"{BANCO}_{_fmt_anios(VENTANA_VAL_AÑOS)}_{_fmt_anios(VENTANA_TEST_AÑOS)}"
