...
Flujo:
  ...
  3. Optuna: optimizar pinball exacta mediana (τ=0.50) en VAL.
  4. Re-entrenar cada quantil con mejores hiperparámetros sobre TRAIN.
...
#          elimina el lookahead que tiene el parquet (GARCH estimado sobre dataset completo)
# False → usa GARCH del parquet tal cual (comportamiento original, internamente consistente)
...
def objective_xgb(
    ...
    """
    Función objetivo para Optuna: pinball loss exacta mediana en validación.
    Se optimiza sobre τ=0.50 — mismos hiperparámetros para todos los cuantiles.
...
def optimizar_hiperparametros(
    ...
    """Ejecuta Optuna y retorna los mejores hiperparámetros (incluye s óptimo)."""
    ...
    study.optimize(
        lambda trial: objective_xgb(
            trial, X_train, y_train, X_val, y_val, std_y, tau=0.50
        ),
        n_trials=n_trials,
...
def guardar_modelos(
    ...
    metadata: features, hiperparámetros (incluye s óptimo), métricas, fecha.
    """
...
def entrenar_banco(banco: str) -> dict | None:
    ...
    # Re-estimar GARCH sin lookahead (opcional)
    ...
    best_params = optimizar_hiperparametros(
        X_train, y_train, X_val, y_val, std_y, N_TRIALS_OPTUNA, banco
    )
