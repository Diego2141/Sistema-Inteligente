...
v5 incorpora 3 nuevos componentes opcionales sobre la infraestructura completa
de v4 (fold generation, metrics, plotting, GARCH, XGBoost quantile regression,
target normalization).  Cada componente se controla con un toggle booleano.

1. Prophet seasonal decomposition  (USAR_PROPHET = True)
   ───────────────────────────────────────────────────────
   Por fold:
     a) Fit Prophet en y_train (datos diarios, fechas de train)
     b) Residuos = y_train − yhat_prophet_insample
     c) XGBoost entrenado en residuos_train en lugar de y_train
     d) Pred final = pred_residuo + yhat_prophet_test
   Config: changepoints=['2016-07-01'], changepoint_prior_scale=0.05,
           yearly/weekly seasonality additive, monthly/quarterly fourier.
   Si Prophet falla, el fold revierte a comportamiento v4 (log + warning).

2. SADF regime features  (USAR_SADF = True)
   ──────────────────────────────────────────
   Computa dos series SADF (Supremum Augmented Dickey-Fuller) como features:
     · sadf_60d  : SADF con ventana_max=60
     · sadf_120d : SADF con ventana_max=120
   Implementación vectorizada con numpy.linalg.lstsq (sin loop interno).
   Calculado rolling sobre la serie completa → val/test reciben su valor natural.

3. HMM regime states  (USAR_HMM = True)
   ──────────────────────────────────────
   GaussianHMM de 3 estados por fold (fit solo en TRAIN):
     · hmm_estado      : estado 0/1/2 (0=calm, 2=severe stress, por varianza)
     · hmm_prob_stress : probabilidad del estado de mayor varianza
   Si hmmlearn falla, el fold continúa sin esas features.

═══════════════════════════════════════════════════════════════════════════════
Infraestructura v4 preservada íntegramente
═══════════════════════════════════════════════════════════════════════════════
  ...
  #4 Normalización del target (NORMALIZAR_TARGET)
  #5 Expanding / Rolling, todos los fan charts, diagnóstico de features
"""
...
DIR_OUTPUT   = BASE_SISTEMA / "2. Output" / "step005_wfcv_v5"
...
S_MIN_FACTOR     = 0.01
S_MAX_FACTOR     = 0.10   # paper recomienda s∈[0.05,0.10] sobre targets estandarizados
...
# grandes. Con NORMALIZAR_TARGET=True los targets tienen std≈1, por lo que
# MAX_DELTA_STEP_FACTOR se aplica directamente (sin multiplicar std_y).
# None → desactivado
MAX_DELTA_STEP_FACTOR = 0.5   # con normalización: 0.5 × 1 = 0.5 (exacto del paper)

# ── Normalización del target ──────────────────────────────────────────────────
# True  → divide y_train/y_val/y_test por std_y antes de entrenar (recomendado
#          paper 2406.02293). XGBoost trabaja en escala [-3,3] sin overflow.
#          s=0.05 y max_delta_step=0.5 se aplican directamente (sin × std_y).
#          Las predicciones se multiplican por std_y al salir.
# False → comportamiento v3 (sin normalización, s y max_delta_step escalan std_y)
...
# ── v5 nuevos componentes ─────────────────────────────────────────────────────
USAR_PROPHET = True    # descomposición estacional con Prophet
...
# Desactivar automáticamente si las librerías no están instaladas
if USAR_PROPHET and not _PROPHET_OK:
    import warnings as _w
    _w.warn("Prophet no está instalado — USAR_PROPHET se desactiva automáticamente", stacklevel=2)
    USAR_PROPHET = False

if USAR_HMM and not _HMM_OK:
    import warnings as _w
    _w.warn("hmmlearn no está instalado — USAR_HMM se desactiva automáticamente", stacklevel=2)
    USAR_HMM = False
...
###############################################################################
# PARTE v5 — SADF / HMM / Prophet
###############################################################################

def _calcular_sadf_serie(values: np.ndarray, tau_min: int = 30,
                          ventana_max: int = 120, lags: int = 1) -> np.ndarray:
    """SADF rolling (Supremum ADF) sobre un array 1-D.
    Solo usa numpy OLS — sin statsmodels. Devuelve NaN para los primeros
    ventana_max puntos."""
    n    = len(values)
    ...
    for t in range(ventana_max, n):
        ...
        for t0 in range(max(lags + 1, t - ventana_max), t - tau_min + 1):
            ...
            if n_dep < 4:
                continue
            cols = [np.ones(n_dep), y_w[lags:-1]]
            for l in range(1, lags + 1):
                cols.append(dy_w[lags - l: T_w - l])
            X_mat = np.column_stack(cols)
            try:
                coeffs, _, rank, _ = np.linalg.lstsq(X_mat, dep, rcond=None)
                if rank < X_mat.shape[1]:
                    continue
                resid = dep - X_mat @ coeffs
                s2 = np.dot(resid, resid) / max(n_dep - X_mat.shape[1], 1)
                if s2 <= 0:
                    continue
...
# Public alias as required by the v5 interface spec
calcular_sadf = _calcular_sadf_serie


def _agregar_sadf_a_df(df: pd.DataFrame) -> pd.DataFrame:
    """Calcula sadf_60d y sadf_120d sobre la serie h=1 (target diario) y
    los añade a df (broadcast a todos los horizontes por fecha_t)."""
    daily = (df[df["h"] == 1][["fecha_t", "target"]]
             .drop_duplicates("fecha_t")
             .sort_values("fecha_t")
             .dropna(subset=["target"]))

    if len(daily) < 130:
        logger.warning("  [SADF] Serie demasiado corta — SADF omitido")
        return df

    vals = daily["target"].values.astype(float)
    # Imputar NaN puntuales por interpolación lineal
    nans = np.isnan(vals)
    if nans.any():
        idx  = np.arange(len(vals))
        vals = np.interp(idx, idx[~nans], vals[~nans])

    logger.info(f"  [SADF] Calculando sadf_60d y sadf_120d ({len(vals):,} puntos) ...")
    t0s = time.time()
    s60  = _calcular_sadf_serie(vals, tau_min=30, ventana_max=60,  lags=1)
    s120 = _calcular_sadf_serie(vals, tau_min=30, ventana_max=120, lags=1)
    logger.info(f"  [SADF] Completado en {time.time()-t0s:.1f}s")

    # Forward-fill NaN iniciales con el primer valor válido
    for arr in (s60, s120):
        mask = ~np.isnan(arr)
        if mask.any():
    ...
    map60  = dict(zip(daily["fecha_t"].values, s60))
    map120 = dict(zip(daily["fecha_t"].values, s120))

    df = df.copy()
    df["sadf_60d"]  = df["fecha_t"].map(map60).astype(float)
    df["sadf_120d"] = df["fecha_t"].map(map120).astype(float)
    return df


def _ajustar_prophet_fold(fechas_train: pd.Series, y_train_vals: np.ndarray,
                           changepoints: list, prior: float):
    """Ajusta Prophet en el set de entrenamiento. Devuelve modelo o None."""
    if Prophet is None:
        return None
    try:
...
def _entrenar_hmm_fold(y_norm: np.ndarray, vol_norm: np.ndarray,
    ...
    donde state_order[0]=calma, state_order[-1]=stress severo, o None si falla."""
    if _hmmlearn_hmm is None:
        return None
    try:
        X = np.column_stack([y_norm, vol_norm])
        model = _hmmlearn_hmm.GaussianHMM(
            n_components=n_states, covariance_type="full",
            n_iter=1000, random_state=42,
        )
        model.fit(X)
        variances  = np.array([model.covars_[s][0, 0] for s in range(n_states)])
        state_order = np.argsort(variances)   # 0=menor varianza (calma)
        return model, state_order
    except Exception as _e:
        logger.warning(f"    [HMM] Error al ajustar: {_e}")
        return None
...
def get_feature_cols(df):
    excluir = COLS_EXCLUIR | {"fecha_th", "target_original"}
    cols = [c for c in df.columns
            if c not in excluir and df[c].dtype.kind in ("f", "i", "u", "b")]
    # Asegurar que columnas v5 se incluyan si existen (ya pasan el filtro de dtype,
    # pero las listamos explícitamente para claridad)
    # sadf_60d, sadf_120d  → float  → ya incluidas
    # hmm_estado           → int    → ya incluida
    # hmm_prob_stress      → float  → ya incluida
    return cols
...
def graficar_metricas_wfcv(df_test_m, banco):
    ...
    plt.tight_layout()
    nombre = DIR_PLOTS / f"wfcv_v5_metricas_{banco}.png"
...
def graficar_comparacion_val_test(df_val_m, df_test_m, banco):
    ...
    for ax, col, titulo in [
        ...
        ax.grid(True, alpha=0.25)
    plt.tight_layout()
    nombre = DIR_PLOTS / f"wfcv_v5_val_vs_test_{banco}.png"
    plt.savefig(nombre, dpi=150, bbox_inches="tight")
    plt.close(fig)
...
def graficar_cobertura_por_h(df_por_h, banco, sufijo="test"):
    ...
    plt.tight_layout()
    nombre = DIR_PLOTS / f"wfcv_v5_cobertura_{sufijo}_{banco}.png"
    plt.savefig(nombre, dpi=150, bbox_inches="tight")
    plt.close(fig)
...
def graficar_hiperparametros_wfcv(df_test_m, banco):
    ...
    for ax in axes_flat[n:]:
        ax.set_visible(False)
    nombre = DIR_PLOTS / f"wfcv_v5_hiperparametros_{banco}.png"
    plt.savefig(nombre, dpi=150, bbox_inches="tight")
    plt.close(fig)
...
def graficar_importancia_por_fold(
    ...
    plt.tight_layout()
    nombre_h = DIR_PLOTS / f"wfcv_v5_importancia_heatmap_{banco}.png"
    ...
    plt.tight_layout()
    nombre_r = DIR_PLOTS / f"wfcv_v5_importancia_ranking_{banco}.png"
    ...
    ruta_csv = DIR_MODO / f"wfcv_v5_importancias_{banco}.csv"
...
def _cargar_metadata_disco(banco: str) -> dict:
    ...
    """
    sfx = "lgbm_wfcv_v5" if MODELO_CV == "lgbm" else f"{MODELO_CV}_wfcv_v5"
...
def _cargar_modelos_step004(banco: str) -> dict | None:
    ...
    if not metas:
        ...
        metas = sorted(DIR_MODELOS_STEP004.glob(f"metadata_{sfx}_wfcv_v5_{banco}_*.json"), reverse=True)
    if not metas:
...
def evaluar_banco(banco: str):
    ...
    # ── v5: Pre-cómputo de SADF (una sola vez, rolling backward — sin leakage) ──
    if USAR_SADF:
        df = _agregar_sadf_a_df(df)
        cols_feat = get_feature_cols(df)   # actualizar con las nuevas columnas

    # Backup de target para restaurar después de residuos Prophet
    if USAR_PROPHET:
    ...
    for fold in folds:
        t_fold     = time.time()
        fold_num_v = fold["fold"]
        logger.info(f"\n  ── Fold {fold_num_v}/{len(folds)} ──────────────────────")

        # ── v5: HMM (per-fold, fit only on train data) ───────────────────
        _hmm_prophet_ok = False    # flag para reconstruir predicciones
        _prophet_model  = None
        _yhat_test_arr  = None
        _yhat_val_arr   = None
        ...
        if USAR_HMM:
            try:
                _mask_h1_tr = ((df["fecha_t"] >= fold["train_start"]) &
                               (df["fecha_t"] <= fold["train_end"]) & (df["h"] == 1))
                _daily_tr   = (df[_mask_h1_tr]
                               .drop_duplicates("fecha_t")
                               .sort_values("fecha_t"))
                _y_d  = _daily_tr["target"].fillna(0).values.astype(float)
                # Usar vol_movil_22d si existe, si no calcularla
                if "vol_movil_22d" in df.columns:
                    _v_d = _daily_tr["vol_movil_22d"].fillna(0).values.astype(float)
                else:
                    _ser = pd.Series(_y_d)
                    _v_d = _ser.rolling(22, min_periods=5).std().fillna(0).values

                _sc_y = float(np.std(_y_d)) or 1.0
                _sc_v = float(np.std(_v_d)) or 1.0
                _hmm_res = _entrenar_hmm_fold(_y_d / _sc_y, _v_d / _sc_v)
                if _hmm_res is not None:
                    _hmm_mod, _state_ord = _hmm_res
                    _stress_s = int(_state_ord[-1])
                    # Predict states for all data in the fold range
                    _mask_h1_all = ((df["fecha_t"] >= fold["train_start"]) &
                                    (df["fecha_t"] <= fold["test_end"]) & (df["h"] == 1))
                    _daily_all   = (df[_mask_h1_all]
                                    .drop_duplicates("fecha_t")
                                    .sort_values("fecha_t"))
                    _y_all  = _daily_all["target"].fillna(0).values.astype(float)
                    if "vol_movil_22d" in df.columns:
                        _v_all = _daily_all["vol_movil_22d"].fillna(0).values.astype(float)
                    else:
                        _v_all = pd.Series(_y_all).rolling(22, min_periods=5).std().fillna(0).values
                    _X_hmm     = np.column_stack([_y_all / _sc_y, _v_all / _sc_v])
                    _states    = _hmm_mod.predict(_X_hmm)
                    _probs     = _hmm_mod.predict_proba(_X_hmm)[:, _stress_s]
                    _ord_map   = {int(_state_ord[i]): i for i in range(len(_state_ord))}
                    _states_r  = np.array([_ord_map[int(s)] for s in _states])
                    _ft_vals   = _daily_all["fecha_t"].values
                    _hmm_e_map = dict(zip(_ft_vals, _states_r.astype(float)))
                    _hmm_p_map = dict(zip(_ft_vals, _probs))
                    df["hmm_estado"]      = df["fecha_t"].map(_hmm_e_map)
                    df["hmm_prob_stress"] = df["fecha_t"].map(_hmm_p_map)
                    cols_feat = get_feature_cols(df)
                    logger.info(f"    [HMM] 3 estados ajustados. "
                                f"Stress_state={_stress_s}")
            except Exception as _e_hmm:
                logger.warning(f"    [HMM] Error: {_e_hmm} — omitiendo HMM este fold")

        # ── v5: Prophet residualization (per-fold) ────────────────────────
        if USAR_PROPHET and not SOLO_REGENERAR_PLOTS:
            try:
                _mask_h1_tr_p = ((df["fecha_t"] >= fold["train_start"]) &
                                  (df["fecha_t"] <= fold["train_end"]) & (df["h"] == 1))
                _daily_tr_p   = (df[_mask_h1_tr_p]
                                 .drop_duplicates("fecha_t")
                                 .sort_values("fecha_t")
                ...
                if len(_daily_tr_p) >= 120:
                    _prophet_model = _ajustar_prophet_fold(
                        _daily_tr_p["fecha_t"].values,
                        _daily_tr_p["target"].values,
                        PROPHET_CHANGEPOINTS,
                        PROPHET_CHANGEPOINT_PRIOR,
                    )
                if _prophet_model is not None:
                    # Get all unique fecha_t in fold range and forecast with Prophet
                    _fold_dates = pd.DatetimeIndex(
                        df[(df["fecha_t"] >= fold["train_start"]) &
                           (df["fecha_t"] <= fold["test_end"])
                        ]["fecha_t"].unique()
                    ).sort_values()
                    _yhat_all = _predecir_prophet(_prophet_model, _fold_dates)
                    _yhat_map = dict(zip(_fold_dates, _yhat_all))

                    # Residualize: target -= yhat_prophet (mapping by fecha_t)
                    _fold_mask = ((df["fecha_t"] >= fold["train_start"]) &
                                  (df["fecha_t"] <= fold["test_end"]))
                    _yhat_rows = df.loc[_fold_mask, "fecha_t"].map(_yhat_map).values
                    df.loc[_fold_mask, "target"] = (
                        df.loc[_fold_mask, "target_original"].values - _yhat_rows
                    )

                    # Store yhat for test and val reconstruction after prediction
                    _test_mask = ((df["fecha_t"] >= fold["test_start"]) &
                                  (df["fecha_t"] <= fold["test_end"]))
                    _yhat_test_arr = df.loc[_test_mask, "fecha_t"].map(_yhat_map).values

                    _val_mask = ((df["fecha_t"] >= fold["val_start"]) &
                                 (df["fecha_t"] < fold["test_start"]))
                    _yhat_val_arr = df.loc[_val_mask, "fecha_t"].map(_yhat_map).values

                    _hmm_prophet_ok = True
                    logger.info(f"    [Prophet] Residuos calculados — "
                                f"rango yhat [{np.nanmin(_yhat_all):,.0f}, "
                                f"{np.nanmax(_yhat_all):,.0f}]")
            except Exception as _e_prop:
                logger.warning(f"    [Prophet] Error: {_e_prop} — usando target original")
                # Restore original target for this fold
                _fold_mask_r = ((df["fecha_t"] >= fold["train_start"]) &
                                (df["fecha_t"] <= fold["test_end"]))
                df.loc[_fold_mask_r, "target"] = df.loc[_fold_mask_r, "target_original"]
                _prophet_model = None
        ...
        try:
            ...
            logger.warning(f"  Fold {fold_num_v}: error preparando datos — {e}")
            # Restore target before continuing
            if USAR_PROPHET and "target_original" in df.columns:
        ...
        if len(X_train) < 200 or len(X_val) < 20 or len(X_test) < 20:
            logger.warning(f"  Fold {fold_num_v}: datos insuficientes — omitiendo")
            if USAR_PROPHET and "target_original" in df.columns:
                _fm2 = ((df["fecha_t"] >= fold["train_start"]) &
                        (df["fecha_t"] <= fold["test_end"]))
                df.loc[_fm2, "target"] = df.loc[_fm2, "target_original"]
        ...
        # ── Normalización del target (v4) ────────────────────────────────────
        if NORMALIZAR_TARGET and std_y > 0:
            y_train_m = y_train / std_y
            y_val_m   = y_val   / std_y
            std_y_m   = 1.0   # s y max_delta_step se aplican directo (paper)
            logger.info(f"    [NORM] target dividido por std_y={std_y:,.0f} → escala ≈ 1")
        else:
            y_train_m = y_train
            y_val_m   = y_val
            std_y_m   = std_y
        ...
        if SOLO_PROPHET:
            ...
            modelos, best_params = entrenar_fold(
                X_train, y_train_m, X_val, y_val_m, std_y_m,
            ...
            if DIAGNOSTICO_FEATURES:
                try:
                    diag_por_fold.append(
                        diagnosticar_fold(modelos, X_val, y_val_m, cols_feat, fold["fold"])
                    )
        ...
        if not SOLO_PROPHET and _hmm_prophet_ok and _prophet_model is not None:
            try:
                ...
                _yhat_by_fecha = {}
                _fold_dates_unique = pd.DatetimeIndex(
                    df[(df["fecha_t"] >= fold["train_start"]) &
                       (df["fecha_t"] <= fold["test_end"])
                    ]["fecha_t"].unique()
                )
                _yhat_fc = _predecir_prophet(_prophet_model, _fold_dates_unique)
                for _fd, _yh in zip(_fold_dates_unique, _yhat_fc):
                    _yhat_by_fecha[pd.Timestamp(_fd)] = float(_yh)

                # Add prophet forecast back to test predictions
                _yh_test = np.array([_yhat_by_fecha.get(pd.Timestamp(ft), 0.0)
                                     for ft in fechas_t_test])
                for tau in preds_test:
                    preds_test[tau] = preds_test[tau] + _yh_test

                # Add prophet forecast back to val predictions (best effort)
                if _yhat_val_arr is not None and len(_yhat_val_arr) > 0:
                    # val rows are ordered by (fecha_t, h); map unique fechas_t
                    _val_fechas_t = df[
                        (df["fecha_t"] >= fold["val_start"]) &
                        (df["fecha_t"] < fold["test_start"])
                    ]["fecha_t"].values
                    _yh_val = np.array([_yhat_by_fecha.get(pd.Timestamp(ft), 0.0)
                                        for ft in _val_fechas_t])
                    if len(_yh_val) == len(preds_val[list(preds_val.keys())[0]]):
                        for tau in preds_val:
                            preds_val[tau] = preds_val[tau] + _yh_val

                # Restore original target for this fold
                _fold_mask_rest = ((df["fecha_t"] >= fold["train_start"]) &
                                   (df["fecha_t"] <= fold["test_end"]))
                df.loc[_fold_mask_rest, "target"] = (
                    df.loc[_fold_mask_rest, "target_original"].values
                )
                logger.info(f"    [Prophet] Componente estacional sumada a predicciones")
            except Exception as _e_rec:
                logger.warning(f"    [Prophet] Error en reconstrucción: {_e_rec}")
                # Ensure target is restored
                _fold_mask_rest = ((df["fecha_t"] >= fold["train_start"]) &
                                   (df["fecha_t"] <= fold["test_end"]))
                df.loc[_fold_mask_rest, "target"] = (
                    df.loc[_fold_mask_rest, "target_original"].values
    ...
    _save(df_test_m,  f"wfcv_v5_metricas_TEST_{banco}_{fecha_hoy}.csv")
    _save(df_val_m,   f"wfcv_v5_metricas_VAL_{banco}_{fecha_hoy}.csv")
    if not df_por_h_t.empty:
        _save(df_por_h_t, f"wfcv_v5_por_h_TEST_{banco}_{fecha_hoy}.csv")
    if not df_por_h_v.empty:
        _save(df_por_h_v, f"wfcv_v5_por_h_VAL_{banco}_{fecha_hoy}.csv")
    ...
    _save(df_test_m[cols_hp_ok], f"wfcv_v5_hiperparametros_{banco}_{fecha_hoy}.csv")
