# -*- coding: utf-8 -*-
"""
aux_fanchart_horizontes_xgb.py
Fan chart del modelo XGBoost para múltiples fechas de origen.

Lógica idéntica a aux_fanchart_horizontes.py (LightGBM), con tres diferencias:
  1. Carga modelos .json (XGBoost) desde modelos_xgb/
  2. Usa xgboost.Booster.predict() en lugar de lightgbm.Booster
  3. Color de bandas: darkorange (igual que step004) para distinguir de LightGBM
...
COLOR_BANDA = "darkorange"  # distingue XGBoost (naranja) de LightGBM (steelblue)


# ── 1. Cargar modelos XGBoost más recientes ───────────────────────────────────
def cargar_modelos(banco: str, dir_modelos: Path) -> tuple[dict, list[str]]:
    ...
    print(f"Modelo XGBoost cargado : {metas[0].name}")
...
def graficar(resultado: pd.DataFrame, fecha_origen: pd.Timestamp, banco: str,
    ...
    titulo_base = (
        f"Fan Chart XGBoost — {banco}  |  Fecha de origen: {fecha_origen.strftime('%d %b %Y')}  |  "
    ...
    nombre = f"fanchart_xgb_{banco}_{fecha_origen.strftime('%Y%m%d')}_h1_h90.png"
